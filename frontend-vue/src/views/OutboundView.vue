<script setup lang="ts">
/**
 * ============================================
 *  出库管理页 — 选做A（出库单 + 防超卖）
 * ============================================
 *
 * 功能：
 * 1. 客户名称
 * 2. 出库明细「表格内直接添加」：与入库页一致（行内编辑 + 草稿持久化 localStorage）
 * 3. 「可用库存」列：选中商品后实时加载该商品各库位的当前库存；
 *    出库数量超过可用库存时前端直接拦截（后端 Redis 预扣 + DB 原子扣减仍兜底）
 * 4. 提交（createOutboundOrder API，携带 requestId 幂等键）
 */
import { ref, computed, watch, onMounted } from 'vue'
import { ElMessage } from 'element-plus'
import {
  createOutboundOrder,
  getProducts,
  getWarehouses,
  getLocations,
  getInventory,
  type Product,
  type Warehouse,
  type Location,
  type InventoryItem,
  type OutboundItemRequest,
} from '@/api'
import PageCard from '@/components/PageCard.vue'

/** 一行出库明细（warehouseId 仅用于级联与展示，提交时剔除） */
interface OutboundItemRow {
  productId?: number
  warehouseId?: number
  locationCode?: string
  quantity: number
}

const customerName = ref('')
const items = ref<OutboundItemRow[]>([])
const submitting = ref(false)

/** 行内编辑态：null=无编辑行，-1=新增，>=0=编辑对应行 */
const editingIndex = ref<number | null>(null)
const draft = ref<OutboundItemRow>({ productId: undefined, warehouseId: undefined, locationCode: undefined, quantity: 1 })

/** 编辑行占位标记（表格数据里用它代表"正在编辑的行"） */
const DRAFT_MARKER = { __draft: true } as const

/** 表格渲染数据：编辑态时把被编辑行替换为标记行（新增则在末尾追加） */
const tableData = computed<Array<OutboundItemRow | typeof DRAFT_MARKER>>(() => {
  if (editingIndex.value === null) return items.value
  const list: Array<OutboundItemRow | typeof DRAFT_MARKER> = items.value.map((r, i) =>
    i === editingIndex.value ? DRAFT_MARKER : r
  )
  if (editingIndex.value === -1) list.push(DRAFT_MARKER)
  return list
})

/**
 * 判断是否为编辑行：用标记字段而非引用比较（行对象可能被 Vue 响应式代理包装）。
 */
const isDraftRow = (row: any): boolean => !!row && row.__draft === true

/** 幂等键：本表单会话的 UUID（弱网重试复用，成功后换新）；crypto.randomUUID 降级实现 */
const genRequestId = (): string => {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return crypto.randomUUID()
  }
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (c) => {
    const r = (Math.random() * 16) | 0
    const v = c === 'x' ? r : (r & 0x3) | 0x8
    return v.toString(16)
  })
}
const requestId = ref<string>(genRequestId())

// 基础数据
const products = ref<Product[]>([])
const warehouses = ref<Warehouse[]>([])
const locationCache = ref<Record<number, Location[]>>({})
const locationLoading = ref<Record<number, boolean>>({})

/** 商品 → 该商品各库位库存行缓存（选做A：可用库存展示） */
const inventoryCache = ref<Record<number, InventoryItem[]>>({})
const inventoryLoading = ref<Record<number, boolean>>({})

/** 加载某商品全部库存行（含库位与数量），用于行内"可用库存"展示 */
const loadInventory = (productId: number) => {
  if (inventoryCache.value[productId] || inventoryLoading.value[productId]) return
  inventoryLoading.value[productId] = true
  getInventory({ productId, pageSize: 100 })
    .then((res) => {
      inventoryCache.value[productId] = res.data.list
    })
    .catch((e: any) => {
      ElMessage.error('加载库存失败: ' + (e.response?.data?.message || e.message))
    })
    .finally(() => {
      inventoryLoading.value[productId] = false
    })
}

/** 某商品某库位的当前可用库存；未知返回 undefined */
const availableQty = (productId?: number, locationCode?: string): number | undefined => {
  if (!productId || !locationCode) return undefined
  const rows = inventoryCache.value[productId]
  if (!rows) return undefined
  const row = rows.find((r) => r.locationCode === locationCode)
  return row ? row.quantity : 0
}

// 展示辅助
const productLabel = (id?: number) => {
  const p = products.value.find((it) => it.id === id)
  return p ? `${p.name} (${p.sku})` : '-'
}
const warehouseLabel = (id?: number) => {
  const w = warehouses.value.find((it) => it.id === id)
  return w ? w.name : '-'
}

onMounted(async () => {
  try {
    const [p, w] = await Promise.all([getProducts(), getWarehouses()])
    products.value = p.data
    warehouses.value = w.data
  } catch (e: any) {
    ElMessage.error('加载基础数据失败: ' + (e.response?.data?.message || e.message))
  }
  restoreDraft()
})

const loadLocations = (warehouseId: number) => {
  if (locationCache.value[warehouseId]) return
  locationLoading.value[warehouseId] = true
  getLocations(warehouseId)
    .then((res) => {
      locationCache.value[warehouseId] = res.data
    })
    .catch((e: any) => {
      ElMessage.error('加载库位失败: ' + (e.response?.data?.message || e.message))
    })
    .finally(() => {
      locationLoading.value[warehouseId] = false
    })
}

// —— 草稿持久化（localStorage） ——

const DRAFT_KEY = 'wms.outbound.draft'

const normalizeRow = (r: any): OutboundItemRow => ({
  productId: typeof r?.productId === 'number' ? r.productId : undefined,
  warehouseId: typeof r?.warehouseId === 'number' ? r.warehouseId : undefined,
  locationCode: typeof r?.locationCode === 'string' && r.locationCode ? r.locationCode : undefined,
  quantity: typeof r?.quantity === 'number' && r.quantity > 0 ? r.quantity : 1,
})

const saveDraft = () => {
  const hasContent =
    customerName.value.trim() !== '' || items.value.length > 0 || editingIndex.value !== null
  if (!hasContent) {
    localStorage.removeItem(DRAFT_KEY)
    return
  }
  localStorage.setItem(
    DRAFT_KEY,
    JSON.stringify({
      customerName: customerName.value,
      items: items.value,
      editingIndex: editingIndex.value,
      draft: draft.value,
      savedAt: Date.now(),
    })
  )
}

const restoreDraft = () => {
  try {
    const raw = localStorage.getItem(DRAFT_KEY)
    if (!raw) return
    const data = JSON.parse(raw)
    if (!data || typeof data !== 'object') return
    if (typeof data.customerName === 'string') customerName.value = data.customerName
    if (Array.isArray(data.items)) items.value = data.items.map(normalizeRow)
    if (
      typeof data.editingIndex === 'number' &&
      (data.editingIndex === -1 || data.editingIndex < items.value.length)
    ) {
      editingIndex.value = data.editingIndex
      draft.value = normalizeRow(data.draft)
      if (draft.value.warehouseId) loadLocations(draft.value.warehouseId)
    }
    // 补加载：已确认行涉及的商品库存与库位（刷新后可编辑/可校验）
    items.value.forEach((it) => {
      if (it.productId) loadInventory(it.productId)
      if (it.warehouseId) loadLocations(it.warehouseId)
    })
    if (customerName.value || items.value.length > 0 || editingIndex.value !== null) {
      ElMessage.info('已恢复上次未提交的草稿')
    }
  } catch {
    localStorage.removeItem(DRAFT_KEY)
  }
}

watch([customerName, items, editingIndex, draft], saveDraft, { deep: true })

// —— 行内编辑 ——

const openAdd = () => {
  if (editingIndex.value !== null) {
    ElMessage.warning('请先保存或取消当前编辑中的行')
    return
  }
  draft.value = { productId: undefined, warehouseId: undefined, locationCode: undefined, quantity: 1 }
  editingIndex.value = -1
}

const openEdit = (index: number) => {
  if (editingIndex.value !== null) {
    ElMessage.warning('请先保存或取消当前编辑中的行')
    return
  }
  draft.value = { ...items.value[index] }
  if (draft.value.warehouseId) {
    loadLocations(draft.value.warehouseId)
  }
  if (draft.value.productId) {
    loadInventory(draft.value.productId)
  }
  editingIndex.value = index
}

const cancelEdit = () => {
  editingIndex.value = null
  draft.value = { productId: undefined, warehouseId: undefined, locationCode: undefined, quantity: 1 }
}

/** 选择商品后重置库位并加载该商品库存（可用库存列） */
const handleProductChange = () => {
  draft.value.locationCode = undefined
  if (draft.value.productId) {
    loadInventory(draft.value.productId)
  }
}

const handleWarehouseChange = () => {
  draft.value.locationCode = undefined
  if (draft.value.warehouseId) {
    loadLocations(draft.value.warehouseId)
  }
}

/** 出库数量是否超过可用库存（已知可用量时前端预校验） */
const exceedsAvailable = (productId?: number, locationCode?: string, quantity?: number): boolean => {
  if (!productId || !locationCode || !quantity) return false
  const avail = availableQty(productId, locationCode)
  return avail !== undefined && quantity > avail
}

const confirmDraft = () => {
  if (!draft.value.productId) {
    ElMessage.warning('请选择商品')
    return
  }
  if (!draft.value.warehouseId) {
    ElMessage.warning('请选择仓库')
    return
  }
  if (!draft.value.locationCode) {
    ElMessage.warning('请选择库位')
    return
  }
  if (!draft.value.quantity || draft.value.quantity < 1) {
    ElMessage.warning('数量必须大于 0')
    return
  }
  if (exceedsAvailable(draft.value.productId, draft.value.locationCode, draft.value.quantity)) {
    ElMessage.warning(
      `可用库存不足：${productLabel(draft.value.productId)} @ ${draft.value.locationCode} 当前仅 ${availableQty(draft.value.productId, draft.value.locationCode)} 件`
    )
    return
  }
  if (editingIndex.value === -1) {
    items.value.push({ ...draft.value })
  } else if (editingIndex.value !== null) {
    items.value[editingIndex.value] = { ...draft.value }
  }
  cancelEdit()
}

const removeItem = (index: number) => {
  items.value.splice(index, 1)
  if (editingIndex.value !== null && editingIndex.value > index) {
    editingIndex.value -= 1
  }
}

// —— 提交 ——

const handleSubmit = async () => {
  if (editingIndex.value !== null) {
    ElMessage.warning('请先保存或取消当前编辑中的行')
    return
  }
  if (!customerName.value.trim()) {
    ElMessage.warning('请填写客户名称')
    return
  }
  if (items.value.length === 0) {
    ElMessage.warning('请至少添加一条出库明细')
    return
  }
  for (let i = 0; i < items.value.length; i++) {
    const it = items.value[i]
    if (!it.productId || !it.warehouseId || !it.locationCode || !it.quantity || it.quantity < 1) {
      ElMessage.warning(`第 ${i + 1} 行明细不完整，请检查后重试`)
      return
    }
    if (exceedsAvailable(it.productId, it.locationCode, it.quantity)) {
      ElMessage.warning(
        `第 ${i + 1} 行可用库存不足：${productLabel(it.productId)} @ ${it.locationCode} 当前仅 ${availableQty(it.productId, it.locationCode)} 件`
      )
      return
    }
  }

  submitting.value = true
  try {
    const payload = {
      customerName: customerName.value.trim(),
      requestId: requestId.value,
      items: items.value.map<OutboundItemRequest>((it) => ({
        productId: it.productId!,
        quantity: it.quantity,
        locationCode: it.locationCode!,
      })),
    }
    const res = await createOutboundOrder(payload)
    const orderNo = (res as any)?.data?.orderNo
    ElMessage.success(orderNo ? `出库单创建成功：${orderNo}` : '出库单创建成功')
    // 成功后重置表单、更换幂等键；watch 自动清除本地草稿
    customerName.value = ''
    items.value = []
    cancelEdit()
    requestId.value = genRequestId()
  } catch (e: any) {
    // 提交失败（如库存不足/弱网超时）：保留 requestId，重试不产生重复单
    ElMessage.error(e.response?.data?.message || '创建出库单失败，请重试')
  } finally {
    submitting.value = false
  }
}
</script>

<template>
  <PageCard title="出库管理">
    <!-- 客户 -->
    <el-form label-width="100px" style="max-width: 640px">
      <el-form-item label="客户名称" required>
        <el-input
          v-model="customerName"
          placeholder="请输入客户名称"
          maxlength="200"
          clearable
          style="width: 320px"
        />
      </el-form-item>
    </el-form>

    <!-- 工具栏 -->
    <div class="table-toolbar">
      <el-button type="primary" :disabled="editingIndex !== null" @click="openAdd">+ 添加明细</el-button>
      <el-button
        type="success"
        :loading="submitting"
        @click="handleSubmit"
        :disabled="items.length === 0"
      >
        提交出库单
      </el-button>
    </div>

    <!-- 明细表格：普通行展示，编辑行直接内嵌控件；"可用库存"列为实时查询 -->
    <el-table :data="tableData" border stripe>
      <el-table-column type="index" label="#" width="55" />
      <el-table-column label="商品" min-width="200">
        <template #default="{ row }">
          <el-select
            v-if="isDraftRow(row)"
            v-model="draft.productId"
            filterable
            placeholder="搜索选择商品"
            style="width: 100%"
            :teleported="true"
            @change="handleProductChange"
          >
            <el-option
              v-for="p in products"
              :key="p.id"
              :label="`${p.name} (${p.sku})`"
              :value="p.id"
            />
          </el-select>
          <span v-else>{{ productLabel(row.productId) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="仓库" width="150">
        <template #default="{ row }">
          <el-select
            v-if="isDraftRow(row)"
            v-model="draft.warehouseId"
            placeholder="选择仓库"
            style="width: 100%"
            :teleported="true"
            @change="handleWarehouseChange"
          >
            <el-option v-for="w in warehouses" :key="w.id" :label="w.name" :value="w.id" />
          </el-select>
          <span v-else>{{ warehouseLabel(row.warehouseId) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="库位" width="150">
        <template #default="{ row }">
          <el-select
            v-if="isDraftRow(row)"
            v-model="draft.locationCode"
            placeholder="选择库位"
            style="width: 100%"
            :teleported="true"
            :loading="!!draft.warehouseId && locationLoading[draft.warehouseId]"
            :disabled="!draft.warehouseId"
          >
            <el-option
              v-for="loc in (draft.warehouseId ? locationCache[draft.warehouseId] || [] : [])"
              :key="loc.code"
              :label="loc.code"
              :value="loc.code"
            />
          </el-select>
          <span v-else>{{ row.locationCode || '-' }}</span>
        </template>
      </el-table-column>
      <!-- 可用库存列（选做A）：实时显示当前库存，不足标红 -->
      <el-table-column label="可用库存" width="110" align="center">
        <template #default="{ row }">
          <span
            v-if="isDraftRow(row)"
            :style="{ color: exceedsAvailable(draft.productId, draft.locationCode, draft.quantity) ? '#f56c6c' : '#67c23a', fontWeight: 600 }"
          >
            {{ draft.productId && draft.locationCode ? (availableQty(draft.productId, draft.locationCode) ?? '-') : '-' }}
          </span>
          <span
            v-else
            :style="{ color: exceedsAvailable(row.productId, row.locationCode, row.quantity) ? '#f56c6c' : '#67c23a', fontWeight: 600 }"
          >
            {{ availableQty(row.productId, row.locationCode) ?? '-' }}
          </span>
        </template>
      </el-table-column>
      <el-table-column label="数量" width="130" align="center">
        <template #default="{ row }">
          <el-input-number
            v-if="isDraftRow(row)"
            v-model="draft.quantity"
            :min="1"
            :max="999999"
            style="width: 100%"
          />
          <span v-else :style="{ color: exceedsAvailable(row.productId, row.locationCode, row.quantity) ? '#f56c6c' : undefined }">
            {{ row.quantity }}
          </span>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="150" align="center">
        <template #default="{ row, $index }">
          <template v-if="isDraftRow(row)">
            <el-button size="small" type="primary" @click="confirmDraft">保存</el-button>
            <el-button size="small" @click="cancelEdit">取消</el-button>
          </template>
          <template v-else>
            <el-button size="small" @click="openEdit($index)">编辑</el-button>
            <el-button size="small" type="danger" @click="removeItem($index)">删除</el-button>
          </template>
        </template>
      </el-table-column>
    </el-table>

    <el-empty
      v-if="items.length === 0 && editingIndex === null"
      description="请点击「+ 添加明细」在表格中直接填写出库商品"
      style="margin-top: 24px"
    />
  </PageCard>
</template>
