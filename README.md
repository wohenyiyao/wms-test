...
# 2) 灌数据（可重复执行，数据重置为种子数据）：
mysql --default-character-set=utf8mb4 -uroot -p wms-test -e "source db/seed.sql"
```

> 后端 JPA（`ddl-auto=update`）启动后会自动对齐表结构（新增列等），与 schema.sql 互不冲突；只重置数据时执行 seed.sql 即可。
...
├── db/schema.sql             # 表结构（9 张表：含 deleted 列/索引/外键，IF NOT EXISTS）
├── db/seed.sql               # 种子数据（清空 + 正式数据）
