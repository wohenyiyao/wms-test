...
**完成情况**：必做 3 项 + 选做 3 项**全部完成**，全部经过自动化测试验证（JUnit 40 用例 + 前端 vitest 7 用例 + smoke 28 用例，全绿）。
...
| Bug 1 策略演进（迭代） | 在 force 级联方案基础上继续优化 | 拍板：**改用逻辑删除**（删除=标记 deleted，不物理删，关联数据全保留） | `@SQLDelete` + `@SQLRestriction` 软删：已删商品查询/引用一律 404，SKU 全局唯一含已删；移除 force 参数。理由：历史可追溯，杜绝孤立数据，比"级联清理"更符合长期数据完整性 |
| Bug 2 修复细节 | 保持页码重新加载 | 选择**保持页码 + 空页回退** | 删除后若当前页为空自动回退一页。理由：删除最后一条时避免停在空页 |

### 5.4 验收结论

任务 3 两个预埋 Bug 均按 §1.1 流程定位并修复：删除策略先修复为关联校验 + 二次确认（事务级联清理防脏数据），后续演进为**逻辑删除**（历史可追溯、已删商品不可见、SKU 全局唯一含已删）；商品页编辑保持页码；回归用例全绿，修复点已在代码注释与 NOTES 中标注。
...
- 联动：`InventoryService.createInboundOrder` 入库成功后同步 Redis 镜像；`ProductService.delete` 演进为逻辑删除（出库记录随历史保留，关联统计方法随之移除）；`InventoryService/InventoryRepository/InventoryController` 查询增加 `productId` 过滤
...
**验收结论：** 出库单 + 双层防超卖闭环完成：JUnit 40 用例（含真实并发集成）+ smoke 28 用例全绿；并发安全由"Redis 预扣闸门 + DB 原子兜底 + 失败补偿 + 序列表发号"四重机制保证；方案与理由、演进方向（定时对账、Redis 预扣的 Lua 门控扩展）已记录。
