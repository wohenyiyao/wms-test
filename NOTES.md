...
> **理解需求 → 设计 → 编码实现 → 检查功能的完善性 → 对照需求文档验收 → 冒烟测试 → 漏洞思考 → 代码 Review**
> （工具链 / 环境配置类问题不在本文赘述）
...
2. **设计**：确定接口契约、事务边界、库存/单号等核心逻辑的落点；
3. **编码实现**：按模板既有风格实现（Lombok + Builder + DTO 转换 + `@Transactional` + `BusinessException`）；
4. **检查功能的完善性**：以用户视角走查功能闭环是否完整 —— 操作后有无明确反馈（成功提示/单号）、空态/加载态/错误提示是否齐全、边界输入与异常路径、页面交互是否可用；**不完整的补全后再进入验收**；
5. **对照需求文档验收**：逐条核对 TASKS.md 考核点与 API_SPEC 字段是否满足（见各任务「验收对照表」）；
6. **冒烟测试**：
...
7. **漏洞思考**：按**固定清单**逐项审视（每个任务都用同一套，见下表）；
   - **影响大 → 修复并做兜底**（含回归用例）；
...
8. **代码 Review**：从四个角度自查并修复 —— **事务安全 / SQL 性能 / 空指针风险 / 并发问题**，修复同时补回归用例。
...
- **协作方式**：理解需求后，按模板风格让 AI 生成初版实现 → 人工审查事务边界、参数校验与边界条件 → 用测试验证 → 发现问题反馈 AI 协助排查。
...
> **功能完善性检查（流程第 4 步）**：以用户视角走查 —— 提交后给出成功提示与入库单号、空态引导（未添加明细）、行内校验提示、异常路径（商品/库位不存在、数量非法）均有明确反馈；闭环完整后再验收。
