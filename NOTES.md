...
| 必做 3 修复 Bug | 商品删除策略：关联校验修复 → 演进为**逻辑删除**（历史可追溯）；编辑保持页码 | 按 §1.1 流程定位，回归用例覆盖 |
...
| 问题 | 解决方案（详见） |
|------|-----------------|
| 预埋 Bug 1：删除有库存商品报 500（无关联校验） | 先按 §1.1 修复为「关联校验 + 二次确认 + force 级联清理」，后演进为**逻辑删除**（标记 deleted，不物理删，历史单据保留可追溯）（§5.1） |
...
| **修复方式** | 分两阶段：**① 先修复为「关联校验 + 二次确认 + force 级联清理」**——删除接口加 `force` 参数（默认 false）：默认统计关联（库存 N 条 / 历史入库记录 N 条），>0 时返回 400 提示数量；前端二次确认后带 `force=true` 重试，force 模式下同事务内级联清理关联数据后再删商品；**② 后演进为逻辑删除**（用户拍板）：删除 = 标记 `deleted`（`@SQLDelete` 将 delete 转为 UPDATE），不物理删除、不清理任何关联数据，`@SQLRestriction("deleted = 0")` 让已删商品在列表/搜索/详情/编辑/入库/出库引用中一律不可见（404）；移除 force 参数，前端保留单次确认框。理由：任务 3 的"孤立库存"问题由"不再物理删除"根治；历史单据与库存保留可追溯；SKU 仍全局唯一（含已删记录，避免历史引用语义混乱） |
| **为何会产生** | 模板为演示"最简删除"省略了业务校验，把引用完整性完全丢给数据库外键，导致用户侧体验是 500 而非友好提示 |
| **回归验证** | `ProductDeleteServiceTest` 5 用例（有库存删除成功且关联保留 / 已删商品查询不可见 / 重复删除 404 / 不存在 404 / 同 SKU 重建被拒）+ smoke 2 用例（有库存逻辑删除 200 + 商品 404、新建无关联删除 200）；`mvn test` 全绿、smoke 全绿 |
