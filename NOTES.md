...
| **根因** | `ProductService.delete()` 预埋点：只校验 `existsById`，**未校验关联库存/入库明细**；直接 `deleteById` 撞上 `inventory`/`inbound_order_items` 的外键约束 → 500。若无外键则会删掉商品留下孤立库存（TASKS 描述的脏数据场景） |
| **修复方式** | 删除接口加 `force` 参数（默认 false）：默认先统计关联（库存 N 条 / 入库明细 N 条），>0 时返回 400 提示关联数量；前端捕获后弹确认框，用户二次确认后带 `force=true` 重试；force 模式下在同一事务内**级联清理**关联库存 + 入库明细后再删商品。无关联时直接删除 |
