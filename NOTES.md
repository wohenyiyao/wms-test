...
**完成情况**：必做 3 项 + 选做 3 项**全部完成**，全部经过自动化测试验证（JUnit 41 用例 + 前端 vitest 7 用例 + smoke 28 用例，全绿）。
...
| 必做 3 修复 Bug | 商品删除策略：关联校验修复 → 演进为**逻辑删除**（历史可追溯，**保留渐进式确认提示**）；编辑保持页码 | 按 §1.1 流程定位，回归用例覆盖 |
...
| 问题 | 解决方案（详见） |
|------|-----------------|
| 预埋 Bug 1：删除有库存商品报 500（无关联校验） | 先按 §1.1 修复为「关联校验 + 二次确认 + force 级联清理」，后演进为**逻辑删除**（标记 deleted，不物理删，历史单据保留可追溯），并**保留渐进式确认提示**（有关联先 400 提示数量，二次确认后才删）（§5.1） |
...
| **修复方式** | 分两阶段：**① 先修复为「关联校验 + 二次确认 + force 级联清理」**——删除接口加 `force` 参数（默认 false）：默认统计关联（库存 N 条 / 历史入库记录 N 条），>0 时返回 400 提示数量；前端二次确认后带 `force=true` 重试，force 模式下同事务内级联清理关联数据后再删商品；**② 后演进为逻辑删除**（用户拍板）：删除 = 标记 `deleted`（`@SQLDelete` 将 delete 转为 UPDATE），不物理删除、不清理任何关联数据，`@SQLRestriction("deleted = 0")` 让已删商品在列表/搜索/详情/编辑/入库/出库引用中一律不可见（404）；**保留渐进式确认提示**（用户要求）：有关联数据时仍先返回 400 提示关联数量与影响范围，前端二次确认后带 `force=true` 才执行删除（force 仅表示"确认"，删除仍是逻辑删除）。理由：任务 3 的"孤立库存"问题由"不再物理删除"根治；历史单据与库存保留可追溯；SKU 仍全局唯一（含已删记录，避免历史引用语义混乱）；渐进式确认让用户清楚删除影响面、防误删 |
| **为何会产生** | 模板为演示"最简删除"省略了业务校验，把引用完整性完全丢给数据库外键，导致用户侧体验是 500 而非友好提示 |
| **回归验证** | `ProductDeleteServiceTest` 6 用例（有关联不带 force → 400 提示数量且商品未删 / force=true 逻辑删除成功且关联保留 / 无关联直接删除 / 已删商品查询不可见 / 不存在 404 / 同 SKU 重建被拒）+ smoke 2 用例（有关联 400 → force 200 → 404、新建无关联删除 200）；`mvn test` 全绿、smoke 全绿 |

### 5.1.1 逻辑删除解决了什么问题

| 问题 | 逻辑删除如何解决 |
|------|-----------------|
| **历史记录不可追溯**（核心） | 原方案（force 级联清理）删除商品会连同库存、历史入库/出库单据**物理删除**，账目无从查证；逻辑删除只标记 `deleted`，**库存与历史单据完整保留、随时可追溯**——对账、审计、客诉排查时仍能看到"这个 SKU 曾经有哪些出入库记录" |
| **孤立脏数据**（任务 3 预埋 Bug 根因） | 物理删除撞外键报 500、或留下无主库存（TASKS 描述的脏数据场景）；逻辑删除不物理删，从根源上不再产生孤立数据 |
| **误删不可恢复** | 物理删除无法撤销；逻辑删除可随时恢复（`deleted` 置回 0 即复活），前端误操作有后悔药 |
| **已删商品被继续引用** | `@SQLRestriction("deleted = 0")` 让已删商品在列表/搜索/详情/编辑/入库/出库引用中一律 404，防止对失效商品的误下单/误扣减 |
| **SKU 语义混乱** | SKU 全局唯一（含已删记录），历史单据的 SKU 引用始终指向同一商品，不会因"重建同 SKU 商品"导致历史数据串档 |
| **删除操作提示不足（防误删）** | **保留渐进式确认提示**：有关联数据时先返回 400 提示「库存 N 条 / 历史入库 N 条 / 出库 N 条，删除后商品不可见、历史记录保留」，前端二次确认后才执行删除——用户删除前清楚影响面，误删概率更低 |
| **启动初始化误判（兼容性，排查中发现）** | `DataInitializer` 用 `productRepository.count()` 判断是否初始化示例数据——该 count 受 `@SQLRestriction` 过滤，若商品被**全部**逻辑删除会误判"无数据"而重复初始化，撞已删 SKU 唯一约束导致启动失败；已改为原生 SQL 计数（含已删记录） |
...
| Bug 1 策略演进（迭代） | 在 force 级联方案基础上继续优化 | 拍板：**改用逻辑删除**（删除=标记 deleted，不物理删，关联数据全保留），并**保留渐进式确认提示**（用户后续要求） | `@SQLDelete` + `@SQLRestriction` 软删：已删商品查询/引用一律 404，SKU 全局唯一含已删；**force 参数保留为"二次确认"语义**（有关联先 400 提示数量，force=true 才执行逻辑删除）。理由：历史可追溯，杜绝孤立数据，比"级联清理"更符合长期数据完整性；渐进式确认让用户删除前清楚影响面 |
| Bug 2 修复细节 | 保持页码重新加载 | 选择**保持页码 + 空页回退** | 删除后若当前页为空自动回退一页。理由：删除最后一条时避免停在空页 |

### 5.4 验收结论

任务 3 两个预埋 Bug 均按 §1.1 流程定位并修复：删除策略先修复为关联校验 + 二次确认（事务级联清理防脏数据），后续演进为**逻辑删除**（历史可追溯、已删商品不可见、SKU 全局唯一含已删），并保留渐进式确认提示（force 仅表示"确认"）；商品页编辑保持页码；回归用例全绿，修复点已在代码注释与 NOTES 中标注。
...
- 联动：`InventoryService.createInboundOrder` 入库成功后同步 Redis 镜像；`ProductService.delete` 演进为逻辑删除（出库记录随历史保留，关联统计保留用于渐进式确认提示）；`InventoryService/InventoryRepository/InventoryController` 查询增加 `productId` 过滤
...
- **JUnit 用例全绿**（选做 A 完成时 39 例，后经逻辑删除迭代，当前全量 41 例）：
...
| 测试清理顺序违反外键 | tearDown 删主表被 `outbound_order_items.order_id` 外键拦截，残留脏数据 | 先删明细再删主表（事务内执行）；商品删除已演进为逻辑删除，无级联顺序问题 |

**验收结论：** 出库单 + 双层防超卖闭环完成：JUnit 41 用例（含真实并发集成）+ smoke 28 用例全绿；并发安全由"Redis 预扣闸门 + DB 原子兜底 + 失败补偿 + 序列表发号"四重机制保证；方案与理由、演进方向（定时对账、Redis 预扣的 Lua 门控扩展）已记录。
...
| 选做 A：出库单 + 库存扣减防超卖 | ✅ 完成（Redis Lua 预扣 + DB 原子兜底双层防线，41 测试 + 28 冒烟全绿，含真实并发集成测试） |
